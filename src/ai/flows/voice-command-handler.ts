...
export async function handleVoiceCommand(input: VoiceCommandInput): Promise<VoiceCommandOutput> {
  const prompt = `Eres un asistente experto en gestionar una despensa y una lista de la compra. El usuario se comunica en español. Todas tus respuestas deben estar en español.
...
- "quita la leche": remove de la lista donde se encuentre. Si está en ambas, prioriza shopping.
- Si el comando no es claro, devuelve una lista de operaciones vacía y una respuesta pidiendo aclaración.

Estado Actual:
Despensa: [${input.pantryList.join(', ')}]
Lista de la compra: [${input.shoppingList.join(', ')}]

Comando del usuario: "${input.command}"
...
Devuelve un objeto JSON con el campo \`operations\` (la lista de operaciones) y \`response\` (un breve mensaje de confirmación).`;
  ...
  try {
    return JSON.parse(text) as VoiceCommandOutput;
  } catch {
    return { operations: [], response: text } as VoiceCommandOutput;
  }
}
